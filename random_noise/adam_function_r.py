...
def adamConstant(params: List[Tensor],
                 grads: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if amsgrad:
            ...
            if count == 0:
                denom = (exp_avg_sq.sqrt() / math.sqrt(bias_correction2)).add_(eps).add(np.random.uniform(0, 0.01))
            else:
...
def adamRandom(params: List[Tensor],
                grads: List[Tensor],
                exp_avgs: List[Tensor],
                exp_avg_sqs: List[Tensor],
                max_exp_avg_sqs: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if amsgrad:
            ...
            if count == 0:
                denom = (exp_avg_sq.sqrt() / math.sqrt(bias_correction2)).add_(eps).add(np.random.uniform(-0.01, 0.01))
            else:
                denom = (exp_avg_sq.sqrt() / math.sqrt(bias_correction2)).add_(eps)
